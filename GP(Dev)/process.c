...
/*	float currentDose = 0;
	...
	*/
	...
	#ifdef MAIN_RANGE //для 38-х счётчиков расчёт по новой формуле
	...
	Debug_SetCurrentSensorsData((uint8*)sensorCnt);
		/*
		memcpy(sensorCntTemp,sensorCnt,sizeof(uint16)*SENSORS);
		
		for(i = 0;i<SENSORS - 1;i++){
				for(j = 0;j<SENSORS - i - 1;j++){
					if(sensorCntTemp[j] > sensorCntTemp[j+1]){
						sMax = sensorCntTemp[j];
						sensorCntTemp[j] = sensorCntTemp[j+1];
						sensorCntTemp[j+1] = sMax;
					}
				}
			}
	Debug_SetSortSensorsData((uint8*)sensorCntTemp);
		*/
